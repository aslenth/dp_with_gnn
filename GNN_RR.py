...
def p_value(epsilon, n=2):
    return np.e ** epsilon / (np.e ** epsilon + n - 1)
def random_response(data,p):
    for i in range(len(data[0])):
        for j in range(len(data[1])):
            value = np.where(data[i][j] == 1, np.random.binomial(1, p), np.random.binomial(1, 1-p))
            data[i][j] = value
...
global matrix_first
matrix_first = np.zeros((2708,2708))
for i in range(5429):
        matrix_first[edges[i, 0]][edges[i, 1]] = 1
def train(epoch):
    global matrix
    global matrix_first
    t = time.time()
